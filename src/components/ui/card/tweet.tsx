...
export const TweetHeader = ({ tweet }: { tweet: EnrichedTweet }) => (
  <div className="flex flex-row justify-between tracking-tight">
    ...
    <a href={tweet.url} target="_blank" rel="noreferrer">
      ...
      <Icon
        name="XLogomark"
        className="size-5 items-start text-foreground transition-all ease-in-out hover:scale-105"
      />
    </a>
  </div>
);
...
export const TweetMedia = ({ tweet }: { tweet: EnrichedTweet }) => (
  <div className="flex flex-1 items-center w-full">
    ...
    {tweet.photos && (
      <div className="relative w-full flex transform-gpu snap-x snap-mandatory gap-2 overflow-x-auto">
    ...
    {!tweet.video &&
      !tweet.photos &&
      // @ts-expect-error card exist
      tweet?.card?.binding_values?.thumbnail_image_large?.image_value.url && (
        <img
          // @ts-expect-error card object exist
...
export const MagicTweet = ({
  ...
  return (
    <div
      className={cn(
        "relative flex size-full max-w-lg flex-col gap-2 overflow-hidden p-4 backdrop-blur-md",
        className,
      )}
      {...props}
    >
